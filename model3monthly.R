...
# parameters i.e   mu, delta and tau
#*******************************************************************************
library("minpack.lm")
...
#--------------------------------- data fitting ------------------------------
.args <- c("preProcessed/Lip_data.rds")
...
# seperates the concatenated name by Capital letters
...
inits <- list(mu=35.7372, tau=1.20578e-03 , delta = 1.2e-03); a <- c(0,1e-7,1e-7); b <- c(100,1, 1)
...
system.time(fit <- nls.lm(par = inits, lower = a, upper = b,
                          ...
                          control = nls.lm.control(nprint=1,  ftol = 1e-3)))
